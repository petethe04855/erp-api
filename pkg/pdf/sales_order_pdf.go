...
// GenerateSalesOrderPDF formats an A4 sales order document matching the
// web print template style (same look & feel as the quotation PDF).
func GenerateSalesOrderPDF(data SalesOrderPDFData) ([]byte, error) {
	...
	// Right Header Box: ใบสั่งขาย
	pdf.SetXY(124, startY)
	pdf.SetFont(fontName, "B", 20)
	pdf.SetTextColor(0, 0, 0)
	pdf.CellFormat(74, 8, "ใบสั่งขาย", "", 1, "R", false, 0, "")

	pdf.SetXY(124, pdf.GetY())
	pdf.SetFont(fontName, "B", 8)
	pdf.SetTextColor(107, 114, 128)
	pdf.CellFormat(74, 4, "SALES ORDER · ต้นฉบับ", "", 1, "R", false, 0, "")
	...
	pdf.SetTextColor(0, 0, 0)
	pdf.CellFormat(186, 4.5, "ลูกค้า / ผู้สั่งซื้อ", "", 1, "L", false, 0, "")
	...
	if data.IncludeVat {
		...
		pdf.CellFormat(lblW, 5, "ยอดรวม (ยังไม่รวมภาษี)", "", 0, "L", false, 0, "")
		pdf.CellFormat(valW, 5, formatBaht(total), "", 1, "R", false, 0, "")
	}
	...
	pdf.SetXY(20, 245)
	pdf.CellFormat(sigW, 5, "ผู้รับสินค้า / ลูกค้า", "", 0, "C", false, 0, "")
	pdf.SetXY(115, 245)
	pdf.CellFormat(sigW, 5, "ผู้ส่งสินค้า / ผู้ขาย", "", 1, "C", false, 0, "")
